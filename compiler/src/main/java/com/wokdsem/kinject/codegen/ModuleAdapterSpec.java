...
import com.wokdsem.kinject.core.binder.WeakSingletonBinder;
...
class ModuleAdapterSpec {
	...
	private static List<MethodSpec> getMethods(String moduleName, List<Provide> provides) {
		...
		for (Provide provide : provides) {
			...
			MethodSpec.Builder builder = MethodSpec
				...
				.addStatement("return new $T($L)", ParameterizedTypeName.get(ClassName.get(getScopeClass(provide.scope)), className),
	...
	private static Class getScopeClass(Provides.Scope scope) {
		switch (scope) {
			case WEAK_SINGLETON:
				return WeakSingletonBinder.class;
			case SINGLETON:
				return SingletonBinder.class;
			case NONE:
			default:
	...
	private static TypeSpec addInternalStatement(Provide provide) {
		...
		code.append(")");
		String statement = code.toString();
		return TypeSpec
