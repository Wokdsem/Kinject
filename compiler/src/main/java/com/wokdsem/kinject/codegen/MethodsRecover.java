...
class MethodsRecover {
	...
	static void getMethods(TypeElement module, Collection<Provide> provides, Collection<Include> includes) throws ProcessorException {
	...
	private static Provide buildProvide(TypeElement module, ExecutableElement provideElement, Provides provides) throws ProcessorException {
		...
		Name provideName = provideElement.getSimpleName();
		Reference reference = new Reference(moduleName.toString(), provideName.toString());
		List<Dependency> dependencies = getDependencies(module, provideElement);
		return new Provide.ProvideBuilder(className.toString(), provides.named(), reference)
			.setDependencies(dependencies)
			.setScope(provides.scope())
			.build();
	}
	...
	private static List<Dependency> getDependencies(Element module, ExecutableElement executableElement) throws ProcessorException {
		...
		for (VariableElement param : executableElement.getParameters()) {
			...
			if (paramType.getKind() != TypeKind.DECLARED) {
				...
				throw new ProcessorException(errMsg, param.getSimpleName(), module.getSimpleName(), executableElement.getSimpleName());
			}
			String tClass = ((TypeElement) ((DeclaredType) paramType).asElement())
				.getQualifiedName()
				.toString();
	...
	private static Include buildInclude(TypeElement module, ExecutableElement includeElement) throws ProcessorException {
		...
		Name moduleId = getReturnType(module, includeElement).getQualifiedName();
		Name moduleName = module.getQualifiedName();
		Name includeName = includeElement.getSimpleName();
		Reference reference = new Reference(moduleName.toString(), includeName.toString());
		return new Include(moduleId.toString(), reference);
	}
