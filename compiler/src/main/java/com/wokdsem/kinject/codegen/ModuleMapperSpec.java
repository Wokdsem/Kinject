...
class ModuleMapperSpec {
	...
	public static TypeSpec getModuleMapperSpec(Map<String, Module> modules, Graph graph, String moduleMapperName) {
	...
	private static FieldSpec getModuleField(Module module) {
		...
		return FieldSpec.builder(fieldClassName, fieldName, PRIVATE, FINAL).build();
	}
	...
	private static MethodSpec getConstructor(Map<String, Module> modules, Graph graph) {
		...
		CodeBlock.Builder statementBuilder = CodeBlock.builder().addStatement(initStatement, fieldName, fieldName);
		for (Include include : graph.includes) {
			...
			String value = String.format("%s.%s()", getFieldName(reference.canonicalModuleName), includeReference.method);
	...
	private static MethodSpec getBinderMethod(Map<String, Module> modules, Graph graph, Map<String, Integer> mapper) {
		CodeBlock.Builder switchFlow = CodeBlock.builder().beginControlFlow("switch(keys.get(tClass) + named)");
		for (Provide provide : graph.provides) {
		...
		CodeBlock switchBlock = switchFlow.endControlFlow().build();
